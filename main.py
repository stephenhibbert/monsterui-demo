...
def ChatMessage(msg, user, loading=False, id=None):
    ...
    msg_cls = (
        "bg-primary text-primary-foreground" if user else "bg-muted"
    ) + " p-3 rounded-lg max-w-[80%]"

    if loading:
        return Div(id=id, cls=cls)(Div("Thinking...", cls=msg_cls + " animate-pulse"))

    # If it's a user message, just show plain text
    if user:
        return Div(cls=cls)(Div(msg, cls=msg_cls))

    # For Claude's responses, use markdown rendering
    return Div(cls=cls)(Div(msg, cls=f"{msg_cls} mt-4 my-4 marked"))
        ...
        Container(
            ...
            Form(
                Div(cls="flex gap-2 mt-4")(
                    ChatInput(), Button("Send", cls=ButtonT.primary, hx_disable="true")
                ),
                hx_post="/chat",
                hx_swap="beforeend",
